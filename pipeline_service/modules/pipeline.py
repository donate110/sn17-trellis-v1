...
from modules.mesh_postprocessing import MeshSmoothingService
...
class GenerationPipeline:
    def __init__(self, settings: Settings = settings):
        ...
        self.mesh_smoother = MeshSmoothingService(settings)
    ...
    async def startup(self) -> None:
        ...
        await self.mesh_smoother.startup()
    ...
    async def shutdown(self) -> None:
        ...
        await self.mesh_smoother.shutdown()
    ...
    async def _edit_image_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                )  # Use prime number for better variation
                set_random_seed(retry_seed)

                logger.info(
                    f"Editing image (attempt {attempt + 1}/{max_retries}, seed: {retry_seed})"
                )

                # Edit the image
                image_edited = self.qwen_edit.edit_image(
                    prompt_image=image, seed=retry_seed
                )

                # Validate quality
                is_valid, quality_metrics = validate_image_quality(
                    image_edited, reference_image=image
                )

                # Calculate quality score (higher is better)
                # Combine multiple metrics into a single score
                variance_score = 1.0 - min(
                    quality_metrics.get("variance", 0) / 10000, 1.0
                )
                extreme_pixel_score = 1.0 - min(
                    quality_metrics.get("extreme_pixel_ratio", 0) * 10, 1.0
                )
                local_var_score = min(
                    quality_metrics.get("avg_local_variance", 0) / 100, 1.0
                )

                quality_score = (
                    variance_score * 0.4
                    + extreme_pixel_score * 0.3
                    + local_var_score * 0.3
                )
                ...
                if is_valid and quality_score >= quality_threshold:
                    logger.success(
                        f"Image editing successful on attempt {attempt + 1} with quality score {quality_score:.3f}"
                    )
                    return image_edited
                ...
                last_exception = e
                logger.warning(
                    f"Error during image editing (attempt {attempt + 1}/{max_retries}): {e}"
                )
                ...
                if attempt == max_retries - 1:
                    if best_image is not None:
                        logger.warning(
                            "Returning best result from previous attempts despite error"
                        )
                        return best_image
                    raise e
    ...
    async def generate_from_upload(self, image_bytes: bytes, seed: int) -> bytes:
        ...
        if image.width < min_size or image.height < min_size:
            logger.warning(
                f"Image size ({image.width}x{image.height}) is below recommended minimum ({min_size}x{min_size})"
            )
        ...
        if isinstance(response.ply_file_base64, bytes):
            ...
            raise ValueError(
                f"Unexpected PLY file type: {type(response.ply_file_base64)}"
            )
    ...
    async def generate_gs(self, request: GenerateRequest) -> GenerateResponse:
        ...
        if image.width < 64 or image.height < 64:
            raise ValueError(
                f"Image too small: {image.width}x{image.height}. Minimum size is 64x64"
            )
        if image.width > 4096 or image.height > 4096:
            logger.warning(
                f"Image very large: {image.width}x{image.height}. This may cause memory issues."
            )
        ...
        if (
            hasattr(self.settings, "enable_image_preprocessing")
            and self.settings.enable_image_preprocessing
        ):
        ...
        if (
            not image_without_background
            or image_without_background.size[0] == 0
            or image_without_background.size[1] == 0
        ):
            logger.warning(
                "Background removal produced invalid image, using edited image instead"
            )
        ...
        min_3d_size = 256
        if (
            image_without_background.width < min_3d_size
            or image_without_background.height < min_3d_size
        ):
            logger.warning(
                f"Image size ({image_without_background.width}x{image_without_background.height}) is below recommended minimum for 3D generation ({min_3d_size}x{min_3d_size})"
            )

        trellis_result = self.trellis.generate(
            TrellisRequest(
                image=image_without_background, seed=request.seed, params=trellis_params
            )
        )

        # Validate 3D generation result

        # 4. Apply mesh smoothing post-processing
        if self.settings.enable_mesh_smoothing:
            logger.info("Applying mesh smoothing post-processing")
            trellis_result.ply_file = self.mesh_smoother.smooth_ply(
        ...
        response = GenerateResponse(
            ...
            image_without_background_file_base64=(
                image_without_background_base64
                if self.settings.send_generated_files
                else None
            ),
        )
